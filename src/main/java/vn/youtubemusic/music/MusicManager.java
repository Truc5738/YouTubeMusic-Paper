...
public final class MusicManager {
    ...
    public synchronized void stop(){playbackGeneration++;Track old=current;if(audioDelivery!=null)audioDelivery.stopCurrent(old);current=null;queue.clear();paused=false;audioFiles.clear();endTasks.values().forEach(f->f.cancel(false));endTasks.clear();}
    public synchronized void pause(){if(current!=null&&!paused){paused=true;ScheduledFuture<?> f=endTasks.remove(current.id());if(f!=null)f.cancel(false);if(audioDelivery!=null)audioDelivery.stopCurrent(current);}}
    public synchronized void resume(){if(current!=null&&paused){paused=false;java.nio.file.Path file=audioFiles.get(current.id());if(file!=null){if(audioDelivery==null)audioDelivery=new JavaAudioDelivery(plugin,this);audioDelivery.prepareAndPlay(new AudioBackend.ResolvedAudio(current,file));}else{audioBackend.resolveAndDownload(current.url()).whenComplete((r,e)->Bukkit.getScheduler().runTask(plugin,()->{synchronized(MusicManager.this){if(e!=null){audioFailed(current,e.getMessage());return;}if(current!=null){audioFiles.put(current.id(),r.audioFile());play(current,r.audioFile());}}}));}}}
    public synchronized void skip(){playbackGeneration++;Track old=current;if(audioDelivery!=null)audioDelivery.stopCurrent(old);if(old!=null){ScheduledFuture<?> f=endTasks.remove(old.id());if(f!=null)f.cancel(false);}next();}
    ...
    public synchronized void remove(int index){if(index<1||index>queue.size())return;Iterator<Track> i=queue.iterator();for(int n=1;n<index;n++)i.next();i.next();i.remove();} public synchronized void clearQueue(){queue.clear();} public synchronized Track current(){return current;} public synchronized Deque<Track> queue(){return new ArrayDeque<>(queue);} public synchronized boolean paused(){return paused;} public synchronized boolean loop(){return loop;} public synchronized boolean shuffle(){return shuffle;} public synchronized boolean radio(){return radio;}
    public synchronized void shutdown(){playbackGeneration++;if(audioDelivery!=null)audioDelivery.stopCurrent(current);endTasks.values().forEach(f->f.cancel(false));endTasks.clear();pool.shutdownNow();if(audioBackend instanceof ApiAudioBackend a)a.shutdown();if(audioDelivery!=null)audioDelivery.shutdown();}
}
