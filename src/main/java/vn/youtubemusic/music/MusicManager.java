...
public final class MusicManager {
    ...
    public synchronized void pause(){if(current!=null&&!paused){paused=true; lyrics.pause(); lyrics.pause();ScheduledFuture<?> f=endTasks.remove(current.id());if(f!=null)f.cancel(false);if(audioDelivery!=null)audioDelivery.stopCurrent(current);}}
    public synchronized void resume(){if(current!=null&&paused){paused=false; lyrics.resume(); lyrics.resume();java.nio.file.Path file=audioFiles.get(current.id());if(file!=null){if(audioDelivery==null)audioDelivery=new JavaAudioDelivery(plugin,this);audioDelivery.prepareAndPlay(new AudioBackend.ResolvedAudio(current,file));}else{audioBackend.resolveAndDownload(current.url()).whenComplete((r,e)->Bukkit.getScheduler().runTask(plugin,()->{synchronized(MusicManager.this){if(e!=null){audioFailed(current,e.getMessage());return;}if(current!=null){audioFiles.put(current.id(),r.audioFile());play(current,r.audioFile());}}}));}}}
    ...
    public LyricsManager lyrics(){return lyrics;} public synchronized void shutdown(){playbackGeneration++; lyrics.shutdown(); lyrics.shutdown();if(audioDelivery!=null)audioDelivery.stopCurrent(current);endTasks.values().forEach(f->f.cancel(false));endTasks.clear();pool.shutdownNow();if(audioBackend instanceof ApiAudioBackend a)a.shutdown();if(audioDelivery!=null)audioDelivery.shutdown();}
}
